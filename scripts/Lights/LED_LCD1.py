...
class SmartTrafficLight:
    def __init__(self, led_info_path, resource_catalog_path):
        ...
        self.pending = None  # struttura: {"mode":..., "priority":..., "duration":..., "direction":..., "warning":...}
    ...
    def notify(self, topic, payload):
        ...
        print(f"[MQTT RECEIVED] Topic: {topic}, Payload: {payload}")  # <<< AGGIUNTA
    ...
    def run(self):
        ...
        while True:
            if self.emergency_queue:
                ...
                last_executed_mode = None  # emergency può ripetersi
            elif self.pending is not None:
                # Esegui pending se è diverso dal precedente o è emergency
                if self.pending["mode"] != last_executed_mode or self.pending["mode"] == "emergency":
                    job = self.pending
                    last_executed_mode = job["mode"]
                    self.pending = None
                else:
                    # Ignora pending identico, vai in standard
...
if __name__ == '__main__':
    ...
    system = SmartTrafficLight(led_info_path, resource_catalog_path)
    system.run()